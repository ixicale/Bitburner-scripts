...
export async function main(ns, target = ns.args[0] || 'n00dles') {
	...
	while (true) {
		if (ns.getServerSecurityLevel(target) > sec_thresh) await ns.weaken(target);
		else if (ns.getServerSecurityLevel(target) > money_thresh) await ns.grow(target);
